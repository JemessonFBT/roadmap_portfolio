...
STATUS_COLORS = {'BACKLOG': ('bg-blue-500', 'border-blue-500'), 'Concluido': ('bg-green-500', 'border-green-500'), 'Discover': ('bg-gray-500', 'border-gray-500'), 'Em análise': ('bg-purple-500', 'border-purple-500'), 'Em andamento': ('bg-yellow-500', 'border-yellow-500'), 'Em homologação': ('bg-cyan-500', 'border-cyan-500'), 'Tarefas pendentes': ('bg-red-500', 'border-red-500'), 'Default': ('bg-gray-400', 'border-gray-400')}
ESFORCO_COLORS = {'Evolução': 'bg-green-200 text-green-800', 'Sustentação': 'bg-indigo-200 text-indigo-800', 'SETUP': 'bg-orange-200 text-orange-800', 'Default': 'bg-gray-200 text-gray-800'}
RISK_LEGEND = {"Concluído": "bg-green-100", "Atrasado": "bg-red-100", "Próximos 10 dias": "bg-yellow-100", "Futuro (>10 dias)": "bg-blue-100"}
...
def load_and_clean_data(uploaded_file):
    ...
    try:
        ...
        df.rename(columns={'chave': 'chave', 'resumo': 'resumo', 'emissor': 'emissor', 'tipo_de_item': 'tipo_item', 'conta_de_esforço': 'conta_esforco', 'status': 'status', 'data_limite': 'data_limite', 'data_real': 'data_real'}, inplace=True)
...
def create_list_item_html(item):
    ...
    <tr class="border-t border-gray-200 bg-white">
        ...
        <td class="p-3 align-middle text-center"><span class="text-xs font-semibold px-2 py-1 rounded-full text-white {status_bg}">{item.get('status', 'N/A')}</span></td>
        <td class="p-3 align-middle text-center"><span class="text-xs font-semibold px-2 py-1 rounded-full {esforco_color}">{item.get('conta_esforco', 'N/A')}</span></td>
        <td class="p-3 align-middle text-center"><span class="text-xs font-semibold px-2 py-1 rounded-full bg-gray-200 text-gray-800">{item.get('tipo_item', 'N/A')}</span></td>
        <td class="p-3 align-middle text-center">{completion_date_html}</td>
    </tr>
    """
...
def create_legends_html():
    ...
    <div class="mt-8 pt-4 border-t-2">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div><h3 class="font-bold text-lg mb-4">Legenda de Risco</h3><div class="flex flex-wrap gap-x-6 gap-y-2 text-sm">{risk_legend_html}</div></div>
            <div><h3 class="font-bold text-lg mb-4">Legenda de Status</h3><div class="flex flex-wrap gap-x-6 gap-y-2 text-sm">{status_legend_html}</div></div>
        </div>
    </div>
    """
...
def generate_issuer_content_html(df_issuer):
    ...
    if not planned_items.empty:
        ...
        for (year, month), group in grouped:
            ...
            # O f-string foi formatado em múltiplas linhas para maior clareza e para evitar erros.
            timeline_html += f"""
            <div class="flex-shrink-0 w-80">
                <div class="text-center py-2 px-4 rounded-lg bg-gray-700 text-white font-bold mb-4">
                    {month_name} {year}
                </div>
    ...
    table_header = """<thead class="bg-gray-50"><tr><th class="p-3 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider" style="width: 50%;">Chave e Resumo</th><th class="p-3 text-center text-sm font-semibold text-gray-500 uppercase tracking-wider">Status</th><th class="p-3 text-center text-sm font-semibold text-gray-500 uppercase tracking-wider">Conta de Esforço</th><th class="p-3 text-center text-sm font-semibold text-gray-500 uppercase tracking-wider">Tipo de Item</th><th class="p-3 text-center text-sm font-semibold text-gray-500 uppercase tracking-wider">Data Conclusão</th></tr></thead>"""
    
    def generate_table_section(items_df):
        if items_df.empty: return "<p class='text-gray-500 px-2'>Nenhum item nesta seção.</p>"
        ...
        return f'<div class="border rounded-lg" style="{scroll_style}"><table class="w-full table-fixed">{table_header}<tbody>{table_body}</tbody></table></div>'
    ...
    unplanned_html = f"""<div><h3 class="text-xl font-bold text-gray-700 mb-4 pb-2 border-b-2">Itens em Aberto</h3>{aberto_html}</div><div class="mt-10"><h3 class="text-xl font-bold text-gray-700 mb-4 pb-2 border-b-2">Itens Concluídos</h3>{concluido_html}</div>"""
    ...
    <div class="presentation-slide">
        ...
        <div class="overflow-x-auto pb-4">{timeline_html}</div>{create_legends_html()}
    </div>
    <div class="presentation-slide">
        <h1 class="text-3xl font-bold text-gray-800">Fila de Itens Não Planejados/Sustentação - {emissor}</h1>
        <h2 class="text-xl text-gray-600 mb-8">Demandas que competem pela capacidade da equipe.</h2>{unplanned_html}
    </div>
    """

def wrap_with_html_shell(content, title):
    return f"""<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>{title}</title><script src="https://cdn.tailwindcss.com"></script><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;700&display=swap" rel="stylesheet"><style>body {{ font-family: 'Inter', sans-serif; background-color: #f0f4f8; }}.presentation-slide {{ background-color: white; padding: 2rem; border-radius: 0.5rem; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1); margin-bottom: 2rem; }}</style></head><body class="p-8">{content}</body></html>"""
...
# --- PASSO 3: LÓGICA PRINCIPAL DA APLICAÇÃO ---
if check_password():
    st.title("Gerador de Roadmaps a partir de CSV")
    st.write("Faça o upload do seu arquivo CSV para gerar os relatórios de roadmap. O arquivo deve conter as colunas: Emissor, Chave, Resumo, Status, Data limite, etc.")
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            zip_buffer.seek(0)
            st.download_button(label="Baixar todos os relatórios (.zip)", data=zip_buffer, file_name="roadmaps.zip", mime="application/zip")
